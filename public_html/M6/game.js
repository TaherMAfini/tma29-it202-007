...
function endGame () {
  erase ();
  context.fillStyle = leftScore >= 20 ? '#BC0000' : '#0000BC';
  context.font = '24px Arial';
  context.textAlign = 'center';
  var winner = 1;
  ...
  context.fillStyle = '#BC0000';
  context.font = '24px Arial';
  context.textAlign = 'left';
  context.fillText ('Score: ' + leftScore, 5, 24);
  context.fillStyle = '#0000BC';
  context.textAlign = 'right';
...
function modifyPaddle(paddle, change) {
    paddle.h += change;
    if(paddle.h > 250) {
        paddle.h = 250;
    }
...
function getRightMultiplier(rPaddle, lPaddle) {
    var multiplier = 1;
    if(rPaddle.h < lPaddle.h) {
        multiplier = Math.floor(lPaddle.h/(rPaddle.h*2));
        if(multiplier < 1) {
            multiplier = 1;
        }
    }
...
function getLeftMultiplier(rPaddle, lPaddle) {
    var multiplier = 1;
    if(lPaddle.h < rPaddle.h) {
        multiplier = Math.floor(rPaddle.h/(lPaddle.h*2));
        if(multiplier < 1) {
            multiplier = 1;
        }
    }
...
function drawMultiplier(rPaddle, lPaddle) {
    let rightMult = getRightMultiplier(rPaddle, lPaddle);
    let leftMult = getLeftMultiplier(rPaddle, lPaddle);

    if(rightMult > 1) {
        context.fillStyle = '#0000BC';
        context.textAlign = 'right';
        context.fillText ('x' + rightMult, canvas.width - 5, 48);
    }

    if(leftMult > 1) {
        context.fillStyle = '#BC0000';
        context.textAlign = 'left';
        context.fillText ('x' + leftMult, 5, 48);
    }
}
...
function draw () {
  ...
  if (ball.x < leftPaddle.x) {
    rightScore+= getRightMultiplier(rightPaddle, leftPaddle);
    resetBall ();
    ball.sX *= -1;
    modifyPaddle(rightPaddle, -10);
    modifyPaddle(leftPaddle, 10);
  } else if (ball.x + ball.w > rightPaddle.x + rightPaddle.w) {
    leftScore+= getLeftMultiplier(rightPaddle, leftPaddle);
    resetBall ();
    ball.sX *= -1;
    modifyPaddle(leftPaddle, -10);
    modifyPaddle(rightPaddle, 10);
  ...
  context.textAlign = 'right';
  context.fillText ('Score: ' + rightScore, canvas.width - 5, 24);
  drawMultiplier(rightPaddle, leftPaddle);
